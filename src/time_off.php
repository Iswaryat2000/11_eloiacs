...
$sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`, `DEPARTMENT`, `FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE 1=1"; // Starting with a 1=1 to simplify conditionals

if ($user_position == "Employee" && $EMP_TEAMLEADER == "Yes") {
    $sql_timeoff .= " AND `DEPARTMENT` = '$EMP_DEPARTMENT'";
...
// Handle button clicks to filter the results
if (isset($_POST['all_button'])) {
    $sql_timeoff .= " AND `FROM_DATE` >= '$currentDate'";
} elseif (isset($_POST['pending_button'])) {
    $sql_timeoff .= " AND `STATUS_LEAVE` = 'Pending' AND `FROM_DATE` >= '$currentDate'";
} elseif (isset($_POST['rejected_button'])) {
    $sql_timeoff .= " AND `STATUS_LEAVE` = 'Rejected' AND `FROM_DATE` >= '$currentDate'";
} elseif (isset($_POST['approved_button'])) {
    $sql_timeoff .= " AND `STATUS_LEAVE` = 'Approved' AND `FROM_DATE` >= '$currentDate'";
}

// Finish the SQL query
$sql_timeoff .= " ORDER BY `FROM_DATE` DESC";
